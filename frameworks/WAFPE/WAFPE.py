...
class WAFPE(Framework):
    ...
    def _hookPostItemsLoop(self):
        ...
        for questionId, opts in self.ResultCache.items():
            ...
            # print(f"[WAFPE] Updating QuestionId: {questionId} with Choices: {selected_choices}")
            # print(f"[WAFPE] Failed notes: {failed_notes}")

            try:
                self.WATools.updateAnswers(questionId, selected_choices, unselectedNotes)
            except Exception as e:
                _warn(f"[WAFPE] Failed to update answers for QuestionId {questionId}: {e}")
