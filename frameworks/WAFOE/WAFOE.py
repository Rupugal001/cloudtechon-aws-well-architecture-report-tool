...
class WAFOE(Framework):
    ...
    def _hookPostItemsLoop(self):
        ...
        for questionId, opts in self.ResultCache.items():
            ...
            print(f"[WAFOE] Updating QuestionId: {questionId} with Choices: {selected_choices}")
            print(f"[WAFOE] Failed notes: {failed_notes}")

            try:
                self.WATools.updateAnswers(questionId, selected_choices, unselectedNotes)
            except Exception as e:
                _warn(f"[WAFOE] Failed to update answers for QuestionId {questionId}: {e}")
