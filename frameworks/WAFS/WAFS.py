...
class WAFS(Framework):
    ...
    def _hookPostItemActivity(self, title, section, checks, comp):
        ...
        if comp == 1:
            if choice_id:
                ...
                self.ResultCache[titleKey]["-1"].append("missing")
        elif comp == -1:
            if choice_id:
                self.ResultCache[titleKey]["-1"].append(choice_id)
            else:
            ...
            if choice_id:
                self.ResultCache[titleKey]["0"].append(choice_id)
            else:
                self.ResultCache[titleKey]["-1"].append("missing")
    ...
    def _hookPostItemsLoop(self):
        ...
        for questionId, opts in self.ResultCache.items():
            ...
            # print(f"[WAFS] Updating QuestionId: {questionId} with Choices: {selected_choices}")
            # print(f"[WAFS] Failed notes: {failed_notes}")

            try:
                self.WATools.updateAnswers(questionId, selected_choices, unselectedNotes)
            except Exception as e:
                _warn(f"[WAFS] Failed to update answers for QuestionId {questionId}: {e}")
