...
class DynamoDbCommon(Evaluator):
    ...
    def _check_pitr_enabled(self):
        try:
            result = self.dynamoDbClient.describe_continuous_backups(TableName = self.tables['Table']['TableName'])
            #Check results of ContinuousBackupStatus (ENABLED/DISABLED)
            if result['ContinuousBackupsDescription']['PointInTimeRecoveryDescription']['PointInTimeRecoveryStatus'] == 'ENABLED':
                self.results['enabledPointInTimeRecovery'] = [1, 'On']
            else:
                self.results['enabledPointInTimeRecovery'] = [-1, 'Off']
        except botocore.exceptions.ClientError as e:
            ecode = e.response['Error']['Code']
            emsg = e.response['Error']['Message']
            print(ecode, emsg)
    ...
    def _check_service_limit_wcu_rcu(self, threshold=80):
        try:
            ...
            desc = self.dynamoDbClient.describe_table(TableName=self.tables['Table']['TableName'])
            read_units = desc["Table"]["ProvisionedThroughput"]["ReadCapacityUnits"]
            write_units = desc["Table"]["ProvisionedThroughput"]["WriteCapacityUnits"]
            ...
            # Check utilization for each data point
            for dp in rcuResult['Datapoints']:
                utilization = (dp['Average'] / read_units) * 100 if read_units > 0 else 0
                if utilization >= threshold:
                    _rcuLimitCount += 1

            for dp in wcuResult['Datapoints']:
                utilization = (dp['Average'] / write_units) * 100 if write_units > 0 else 0
                if utilization >= threshold:
                    _wcuLimitCount += 1
                
            # RCU result
            if _rcuLimitCount > 0:
                self.results['rcuServiceLimit'] = [-1, f'RCU utilization > {threshold}% in {_rcuLimitCount} hours over past 7 days.']
            elif rcuResult['Datapoints']:
                self.results['rcuServiceLimit'] = [1, f'RCU utilization <= {threshold}% in past 7 days.']
            else:
                self.results['rcuServiceLimit'] = [0, 'No RCU data available (table may be unused).']

            # WCU result
            if _wcuLimitCount > 0:
                self.results['wcuServiceLimit'] = [-1, f'WCU utilization > {threshold}% in {_wcuLimitCount} hours over past 7 days.']
            elif wcuResult['Datapoints']:
                self.results['wcuServiceLimit'] = [1, f'WCU utilization <= {threshold}% in past 7 days.']
            else:
                self.results['wcuServiceLimit'] = [0, 'No WCU data available (table may be unused).']
    ...
    def _check_dynamodb_autoscaling_enabled(self):
        try:
            ...
            if not results:
                self.results["DynamoDbAutoScalingEnabled"] = [
                    -1,
                    "No scalable targets found (AutoScaling not enabled)"
                ]
                return

            findings = []
            for eachScalableTarget in results:
                res_id = eachScalableTarget["ResourceId"]  # e.g. table/MyTable or table/MyTable/index/MyIndex
                min_cap = eachScalableTarget.get("MinCapacity")
                max_cap = eachScalableTarget.get("MaxCapacity")

                if min_cap is not None and max_cap is not None and min_cap < max_cap:
                    findings.append([1, f"Auto-scaling enabled for {res_id} (min {min_cap}, max {max_cap})"])
                else:
                    findings.append([-1, f"Auto-scaling NOT properly configured for {res_id}"])

            # Store list of results
            self.results["DynamoDbAutoScalingEnabled"] = findings

        except botocore.exceptions.ClientError as e:
            ecode = e.response["Error"]["Code"]
            emsg = e.response["Error"]["Message"]
            self.results["DynamoDbAutoScalingEnabled"] = [-1, f"Error: {ecode} - {emsg}"]
    ...
    def _checkDynamodbEncryption(self):
        table = self.tables["Table"]
        sse = table.get("SSEDescription")

        if not sse or sse.get("Status") != "ENABLED":
            self.results["DynamoDbEncryptionAtRest"] = [-1, f"Table {table['TableName']} not encrypted"]
        else:
            self.results["DynamoDbEncryptionAtRest"] = [1, f"Table {table['TableName']} is encrypted with {sse.get('KMSMasterKeyArn','default key')}"]