...
class Iam(Service):
    def __init__(self, region):
        ...
        self.awsClients = {
            'iamClient': self.iamClient, # AWS Identity and Access Management
            'orgClient': ssBoto.client('organizations'), # AWS Organizations
            'accClient': ssBoto.client('account', config=self.bConfig), # AWS Account Management
            'sppClient': ssBoto.client('support', config=self.bConfig), # AWS Support (Business/Enterprise only)
            'gdClient': ssBoto.client('guardduty', config=self.bConfig), # Amazon GuardDuty
            'budgetClient': ssBoto.client('budgets', config=self.bConfig), # AWS Budgets
            'curClient': ssBoto.client('cur', config=self.bConfig), # AWS Cost and Usage Report
            'ctClient': ssBoto.client('cloudtrail', config=self.bConfig), # AWS CloudTrail
            'shClient': ssBoto.client('securityhub', config=self.bConfig), # AWS Security Hub
            'configClient': ssBoto.client('config', config=self.bConfig), # AWS Config
            'ecsClient': ssBoto.client('ecs', config=self.bConfig), # Amazon Elastic Container Service
            'smClient': ssBoto.client('secretsmanager', config=self.bConfig), # AWS Secrets Manager
            'emrClient': ssBoto.client('emr', config=self.bConfig), # Amazon EMR
            'dmsClient': ssBoto.client('dms', config=self.bConfig), # AWS Database Migration Service
            'ssmClient': ssBoto.client('ssm', config=self.bConfig), # Amazon Simple Systems Manager
            'sageMakerClient': ssBoto.client('sagemaker', config=self.bConfig), # Amazon SageMaker
            'esClient': ssBoto.client('es', config=self.bConfig), # Amazon Elasticsearch Service
            'ebsClient': ssBoto.client('elasticbeanstalk', config=self.bConfig), # Amazon Elastic Beanstalk
            'ecrClient': ssBoto.client('ecr', config=self.bConfig), # Amazon Elastic Container Registry
            'backupClient': ssBoto.client('backup', config=self.bConfig), # AWS Backup
            'acmClient': ssBoto.client('acm', config=self.bConfig), # AWS Certificate Manager
            'accessanalyzerClient': ssBoto.client('accessanalyzer', config=self.bConfig), # AWS Access Analyzer
            'auditmanagerClient': ssBoto.client('auditmanager', config=self.bConfig) # AWS Audit Manager
        }
