...
class CloudwatchCommon(Evaluator):
    ...
    def __init__(self, log, logClient, cwClient):
        super().__init__()
        self.init()
    ...
    def _checkCloudWatchAlarms(self):
        ...
        response = self.cwClient.describe_alarms()
        alarms = response.get('MetricAlarms', [])
        associated = []
        for alarm in alarms:
            dimensions = alarm.get('Dimensions', [])
            for dimension in dimensions:
                if dimension['Name'] == 'LogGroupName' and dimension['Value'] == self.log['logGroupName']:
                    associated.append(alarm['AlarmName'])
        if len(associated) == 0:
            self.results['CloudWatchAlarms'] = [-1, 0]
        else:
            self.results['CloudWatchAlarms'] = [1, len(associated)]