...
class CloudwatchTrails(Evaluator):
    ...
    def _checkCWLoggroup_encryption(self):
        kmsKeys = []
        for lg in self.logClient.describe_log_groups()["logGroups"]:
            if lg.get("kmsKeyId"):
                kmsKeys.append(lg["kmsKeyId"])
        if not kmsKeys:
            self.results['CWLogGroupEncryption'] = [-1, "Log group is not encrypted"]
        else:
            self.results['CWLogGroupEncryption'] = [1, "Log group is encrypted"]
