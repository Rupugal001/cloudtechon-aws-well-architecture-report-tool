...
class S3Bucket(Evaluator):
    ...
    def _check_s3_ssl_only(self):
        self.results['S3SslOnly'] = [-1, 'Off']
        try:
            resp = self.s3Client.get_bucket_policy(
                Bucket=self.bucket
            )['Policy']
            policy = json.loads(resp)
            for statement in policy['Statement']: 
                ...
                if statement['Effect'] == "Deny" and secure_transport == "false":
                    self.results['S3SslOnly'] = [1, 'On']
                    return
                elif statement['Effect'] == "Allow" and secure_transport == "true":
                    self.results['S3SslOnly'] = [1, 'On']
                    return
        except (botocore.exceptions.ClientError, json.JSONDecodeError) as e:
            if isinstance(e, botocore.exceptions.ClientError) and e.response['Error']['Code'] == 'NoSuchBucketPolicy':
                return
            print("[{}] Unable to get Bucket Policy Information, skip".format(self.bucket))
            return
