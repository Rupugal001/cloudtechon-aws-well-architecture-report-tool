...
class GuarddutyDriver(Evaluator):
    ...
    def _checkguardduty_delegated_admin(self):
        try:
            admin = self.gd_client.list_organization_admin_accounts()["AdminAccounts"]
            if admin:
                self.results['DelegatedAdmin'] = [1, admin]
            else:
                self.results['DelegatedAdmin'] = [-1, 'No delegated admin found']
        except ClientError:
            self.results['DelegatedAdmin'] = [-1, 'No delegated admin found']