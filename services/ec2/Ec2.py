...
from services.ec2.drivers.EC2SSMManage import EC2SSMManage
...
class Ec2(Service):
    ...
    def __init__(self, region):
        ...
        self.codebuildClient = ssBoto.client('codebuild', config=self.bConfig)
    ...
    def getSSMManagedInstances(self):
        ...
        result = self.ssmClient.describe_instance_information(
            Filters = filters
        )
        
        instanceList = result.get('InstanceInformationList')
        while result.get('NextToken') is not None:
            result = self.ssmClient.describe_instance_information(
                Filters = filters,
                NextToken = result.get('NextToken')
            )
            instanceList = instanceList + result.get('InstanceInformationList')
        
        return instanceList
    
    # # Get Projects
    # def getCodeBuildProjects(self):
    #     result = self.codebuildClient.list_projects()['projects']
    #     return result
    ...
    def advise(self):
        ...
        ssmInstances = self.getSSMManagedInstances()
        for ssmInstance in ssmInstances:
            _pi('SSM Managed Instance', ssmInstance['InstanceId'])
            obj = EC2SSMManage(ssmInstance, self.ssmClient, self.ec2Client, self.codebuildClient)
            obj.run(self.__class__)
            objs[f"SSM Managed::{ssmInstance['InstanceId']}"] = obj.getInfo()
            
        # # Code Build Projects
        # codebuildProjects = self.getCodeBuildProjects()
        # for projectName in codebuildProjects:
        #     _pi('CodeBuild Project', projectName)
        #     obj = EC2SSMManage({'InstanceId': projectName}, self.ssmClient, self.ec2Client, self.codebuildClient)
        #     obj.run(self.__class__)
        #     objs[f"CodeBuild Project::{projectName}"] = obj.getInfo()
            
        #EBS checks
        volumes = self.getEBSResources()
        for volume in volumes:
            _pi('EBS', volume['VolumeId'])
            obj = Ec2EbsVolume(volume,self.ec2Client, self.cwClient,self.region)
            obj.run(self.__class__)
