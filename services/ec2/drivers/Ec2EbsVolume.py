...
class Ec2EbsVolume(Evaluator):
    ...
    def __init__(self, ebsVolumeData,ec2Client,cwClient,region):
        super().__init__()
        ...
        self.account = sts.get_caller_identity()["Account"]
        self.region = region
    ...
    def _checkEBSBackup(self):
        volume_id = self.ebsVolumeData['VolumeId']

        try:
            # List backup selections across all backup plans
            plans = self.backupClient.list_backup_plans()['BackupPlansList']
            ...
            for plan in plans:
                ...
                selections = self.backupClient.list_backup_selections(
                    BackupPlanId=plan_id
                )['BackupSelectionsList']

                for selection in selections:
                    # Check if resource ARN is included in backup selection
                    resources = selection.get('Resources', [])
                    conditions = selection.get('ListOfTags', [])

                    if f"arn:aws:ec2:{self.region}:{self.account}:volume/{volume_id}" in resources:
                        in_backup_plan = True
                        attached_plans.append(plan['BackupPlanName'])

            if in_backup_plan:
                self.results['EBSBackup'] = [1, f"Volume {volume_id} is in backup plan(s): {attached_plans}"]
            else:
                self.results['EBSBackup'] = [-1, f"Volume {volume_id} is not in any backup plan"]

        except Exception as e:
            self.results['EBSBackup'] = [-1, f"Error checking backup plans: {str(e)}"]
    ...
    def _checkSnapshot(self):
        ...
        if len(snapshotData['Snapshots']) > 0:
            ...
            if launchDay > 7:
                ...
                self.results['EBSUpToDateSnapshot'] = [1, 'Snapshot is up to date.']
        else:
    ...
    def _checkEC2VolumeInuse(self):
        vol_id = self.ebsVolumeData['VolumeId']
        state = self.ebsVolumeData['State']
        
        if state == 'available':
            self.results['EBSNotAttached'] = [-1, "Unused volume (available but not attached)"]
        else:
            self.results['EBSNotAttached'] = [1, f"State: {state}"]
            
    def _checkEC2InBackupPlan(self):
        
        backup = self.backupClient.list_protected_resources()['Results']
        for res in backup:
            if res['ResourceType'] == 'EC2':
                self.results['EBSBackup'] = [1, 'In backup plan']
            else:
                self.results['EBSBackup'] = [-1, 'Not in backup plan']
