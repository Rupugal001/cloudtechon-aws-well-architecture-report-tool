...
class Ec2AutoScaling(Evaluator):
        ...
        if 'LaunchConfigurationName' in asg:
            ...
            for config in result['LaunchConfigurations']:
                ...
                if config.get('MetadataOptions') is None:
                    ...
                    http_tokens = config['MetadataOptions'].get('HttpTokens')
                    if http_tokens == 'optional':
                        self.results['ASGIMDSv2'] = [-1, 'Optional (Not Enforced)']
                    elif http_tokens == 'required':
                        self.results['ASGIMDSv2'] = [1, 'Required (IMDSv2 Enforced)']
                    else:
                        self.results['ASGIMDSv2'] = [-1, f'Unexpected value: {http_tokens}']
                if config.get('AssociatePublicIpAddress') == True:
                    self.results['ASGPublicIP'] = [-1, '']
                else:
                    self.results['ASGPublicIP'] = [1, 'No public IP']
    ...
    def _check_clb_multi_az(self):
        ...
        for lb in result['LoadBalancerDescriptions']:
            if lb['LoadBalancerName'] in asg['LoadBalancerNames']:
                if lb.get('AvailabilityZones') and len(lb['AvailabilityZones']) > 1:
                    return

        self.results['ASGClassicLBMultiAZ'] = [-1, asg['LoadBalancerNames']]
        return