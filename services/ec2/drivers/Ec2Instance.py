...
class Ec2Instance(Evaluator):
    ...
    def _checkIamProfileAssociated(self):
        if "IamInstanceProfile" not in self.ec2InstanceData:
            ...
            self.results['EC2IamProfile'] = [1, self.ec2InstanceData['IamInstanceProfile']]
        return
    ...
    def _checkEC2PublicIP(self):
        ...
        if public_ip:
            # Negative: instance has a public IP
            self.results['EC2InstancePublicIP'] = [-1, public_ip]

            try:
                addrResp = self.ec2Client.describe_addresses(PublicIps=[public_ip])
            except botocore.exceptions.ClientError as e:
                if e.response['Error']['Code'] == 'InvalidAddress.NotFound':
                    # Auto-assigned public IP
                    self.results['EC2InstanceAutoPublicIP'] = [-1, public_ip]
                else:
                    raise
        else:
            # Positive: no public IP attached
            self.results['EC2InstancePublicIP'] = [1, "No Public IP"]
    ...
    def _checkEC2Stopped(self):
        instance = self.ec2InstanceData
        state = instance['State']['Name']
        
        if state == 'stopped':
            self.results['EC2Stopped'] = [-1, 'EC2 is stopped']
        else:
            self.results['EC2Stopped'] = [1, '']
        
        return
    
    def _checkSGAttachedToENI(self):
        instance = self.ec2InstanceData
        eniList = instance.get('NetworkInterfaces', [])

        if not eniList:
            return  # No ENIs, skip check

        sgSet = set()
        for eni in eniList:
            for sg in eni.get('Groups', []):
                sgSet.add(sg.get('GroupId'))

        if not sgSet:
            self.results['EC2ENISG'] = [-1, 'No SG attached to ENI']
        else:
            self.results['EC2ENISG'] = [1, f'Security Groups attached: {list(sgSet)}']

        return

    def _checkEC2MultiENI(self):
        instance = self.ec2InstanceData
        eniList = instance.get('NetworkInterfaces', [])
        ...
        if len(eniList) > 1:
            self.results['EC2MultiENI'] = [-1, 'Multiple ENIs attached']
        else:
            self.results['EC2MultiENI'] = [1, 'Single ENI attached']

        return

    def _checkVPNTunnels(self):
        instance = self.ec2InstanceData
        vpnList = instance.get('VpnConnections', [])

        if not vpnList:
            return  # No VPN tunnels, skip check

        # Initialize tunnel up count
        up_tunnels = 0

        # Iterate over the VPN connections and check the tunnel status
        for vpn in vpnList:
            # Extract VGW telemetry (tunnel status info)
            tunnel_info = vpn.get('VgwTelemetry', [])
            
            for tunnel in tunnel_info:
                if tunnel.get('Status') == 'UP':
                    up_tunnels += 1

        # Validate if both tunnels are up
        if up_tunnels == 2:
            self.results['EC2MultiVPN'] = [1, 'Both VPN tunnels are up (HA confirmed)']
        elif up_tunnels == 1:
            self.results['EC2MultiVPN'] = [0, 'Single VPN tunnel is up']
        else:
            self.results['EC2MultiVPN'] = [-1, 'No VPN tunnels are up or not configured correctly']
            
    def _checkNonOverLappingVPCS(self):
        instance = self.ec2InstanceData
        vpcId = instance.get('VpcId')

        if not vpcId:
            return  # No VPC associated, skip check

        try:
            # Get all VPCs in the account/region
            vpcs = self.ec2Client.describe_vpcs()['Vpcs']
            cidrs = [(v['VpcId'], ipaddress.ip_network(v['CidrBlock'])) for v in vpcs]

            overlaps = []
            for i in range(len(cidrs)):
                for j in range(i + 1, len(cidrs)):
                    if cidrs[i][1].overlaps(cidrs[j][1]):
                        overlaps.append((
                            cidrs[i][0], str(cidrs[i][1]),
                            cidrs[j][0], str(cidrs[j][1])
                        ))

            if overlaps:
                self.results['EC2NonOverlappingVPCs'] = [-1, f'Overlapping CIDRs found: {overlaps}']
            else:
                self.results['EC2NonOverlappingVPCs'] = [1, 'No overlapping VPC CIDRs']

        except botocore.exceptions.ClientError as e:
            self.results['EC2NonOverlappingVPCs'] = [-1, f'Error retrieving VPC info: {str(e)}']

        return
    
    def _checkAutoRecovery(self):
        """Check if Auto Recovery is enabled for the ec2"""
        response = self.ec2Client.describe_instance_maintenance_options(InstanceIds=[self.ec2InstanceData['InstanceId']])

        options = response['InstanceMaintenanceOptionsSet'][0]
        
        if options.get('AutoRecovery') == 'enabled':
            self.results['EC2AutoRecovery'] = [1, 'Auto Recovery is enabled']
        else:
            self.results['EC2AutoRecovery'] = [-1, 'Auto Recovery is not enabled']