...
class RdsCommon(Evaluator):
    ...
    def _checkPublicSnapshot(self):
        print("Checking Public Snapshot")
        ...
        print("Found {} public snapshots".format(len(publicSnapshots)))
        if len(publicSnapshots) > 0 or publicSnapshots is None:
            self.results['SnapshotRDSIsPublic'] = [-1, "At least " + str(len(publicSnapshots))]
        else:
            self.results['SnapshotRDSIsPublic'] = [1, 'No public snapshot']
    ...
    def _check_rds_backup_enabled(self):
        
        if self.db['BackupRetentionPeriod'] == 0:
            self.results['BackupEnabled'] = [-1, 'Off']
        else:
            self.results['BackupEnabled'] = [1, 'On']
