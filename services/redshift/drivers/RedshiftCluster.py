...
class RedshiftCluster(Evaluator):
    ...
    def _checkCluster(self):
        ...
        if self.cluster.get('PubliclyAccessible', False):
            ...
            self.results['PubliclyAccessible'] = [1, "Redshift cluster is not publicly accessible"]
        ...
        if retention_period < 7:
            ...
            self.results['AutomaticSnapshots'] = [1, f"Automatic snapshot retention is {retention_period} days"]
        ...
        if not snapshot_copy.get('DestinationRegion'):
            ...
            self.results['CrossRegionSnapshots'] = [1, f"Cross-region snapshots to {snapshot_copy['DestinationRegion']}"]
        ...
        if not self.cluster.get('PreferredMaintenanceWindow'):
            ...
            self.results['MaintenanceWindow'] = [1, f"Maintenance window is {self.cluster['PreferredMaintenanceWindow']}"]
        ...
        if not self.cluster.get('AllowVersionUpgrade', False):
            ...
            self.results['AutomaticUpgrades'] = [1, "AllowVersionUpgrade is enabled"]
        ...
        if not self.cluster.get('EnhancedVpcRouting', False):
            ...
            self.results['EnhancedVpcRouting'] = [1, "EnhancedVpcRouting is enabled"]
        ...
        if az_status != 'enabled':
            ...
            self.results['AZRelocation'] = [1, "AZ Relocation is enabled"]
